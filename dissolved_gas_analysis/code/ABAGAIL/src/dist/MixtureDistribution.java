...
public class MixtureDistribution extends AbstractDistribution implements Copyable {
    ...
    /*public int getClusterNumber(Instance data){
		...
		Instance mode = mode(data);
		Instance sample = sample(data);
		System.out.println(data);
		System.out.println(mode);
		System.out.println(sample);
		System.out.println(" ");
		for (int i=0; i < components.length; i++){
			System.out.println(components[i].mode());
			System.out.println(components[i].mode(data));
			System.out.println(components[i].mode(mode));
			System.out.println(components[i].mode(sample));
		}
		System.out.println(" ");
		System.out.println(" ");
		//int match = -1;
		//double difference = 1e20;
		//for (int i = 0;i < k;i++){
		//	if (distanceMeasure.value(temp, clusterCenters[i]) < difference){
		//		match = i;
		//		difference = distanceMeasure.value(temp, clusterCenters[i]);
		//	}
		//}
		return componentDistribution.mode(data).getDiscrete();
	}  */ 
