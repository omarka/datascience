...
public class EMClusterer extends AbstractConditionalDistribution implements FunctionApproximater {
    ...
    public int getClusterNumber(Instance data){
		Distribution[] mgd = mixture.getComponents();
		//DiscreteDistribution dd = mixture.getComponentDistribution();
		
		//System.out.println(mgd[0]);
				
		//System.out.println(distributionFor(data).mode());
		
		int temp = distributionFor(data).mode().getDiscrete();
		//return (int)distributionFor(data).mode();
		
		//System.out.println(distributionFor(data).getMean());
		return temp;
		//return mixture.getClusterNumber(data);
		//return distributionFor(data).getClusterNumber();
	}
	
	public double getError(DataSet comparison){
		DistanceMeasure distanceMeasure = new EuclideanDistance();
		Distribution[] comps = mixture.getComponents();
		double error = 0.0;
		for (int i = 0; i < comparison.size(); i++){
			Instance a = comparison.get(i);
			//Instance b = mixture.getClusterCenters()[this.value(comparison.get(i)).getDiscrete()];
			int bIndex = distributionFor(comparison.get(i)).mode().getDiscrete();
			Instance b = comps[bIndex].mode();
			error += Math.pow(distanceMeasure.value(a,b),2);
		}
		//	error += Math.pow(distanceMeasure.value(comparison.get(i), this.value(comparison.get(i))),2); 
		error /= comparison.size();   
		return error;
	}
